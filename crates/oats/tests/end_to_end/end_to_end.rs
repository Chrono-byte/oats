...
fn test_add_example_end_to_end() -> Result<()> {
    ...
    let workspace_root = manifest_dir.parent().and_then(|p| p.parent()).expect("failed to find workspace root");
    ...
    cmd.arg(example).env("OATS_OUT_DIR", out_dir).current_dir(workspace_root);
    ...
    let out = String::from_utf8_lossy(&run_output.stdout).trim().to_string();
    let v: f64 = out.parse()?;
