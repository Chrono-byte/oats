...
use ast::{Callee, AssignTarget, SimpleAssignTarget, MemberProp};
...
impl<'a> crate::codegen::CodeGen<'a> {
    pub(super) fn lower_call_expr(
        &self,
        _call: &deno_ast::swc::ast::CallExpr,
    ...
    // For strings, we always store an i8* pointer.
    // The pointer is either directly to the string data
    // or to a heap-allocated temporary if the original
    // value was a non-escaping literal.
    let store_ptr = if let BasicValueEnum::PointerValue(pv) =
    new_val
    {
    // If the new value is a pointer, store it directly
    pv
    } else {
    // Otherwise, the value must be a string literal
    // or a temporary; we need to allocate a new
    // string object and copy the data.
    let alloc_fn = self.get_string_alloc();
    let cs = match self.builder.build_call(
    alloc_fn,
    &[new_val.into()],
    "string_alloc_copy",
    ...
    // The result is a pointer to the new string object
    bv.into_pointer_value()
    }
    _ => {
    return Err(Diagnostic::simple(
    "string alloc returned non-pointer",
    ));
    }
    }
    };
    // Store the pointer into the computed field location
    let field_ptr_ty = self
    .context
    .ptr_type(inkwell::AddressSpace::default());
    ...
    .builder
                                    .build_pointer_cast(
                                        field_ptr,
                                        field_ptr_ty,
                                        "field_ptr_cast_store",
                                    )
                                    .map_err(|_| {
                                        Diagnostic::simple("LLVM builder error")
                                    })?;
                                let _ = self.builder.build_store(field_ptr_cast, store_ptr);
                                return Ok(store_ptr);
                            }
                            _ => {
                                return Err(Diagnostic::simple("unsupported field type"));
                            }
                        }
                    } else {
                        return Err(Diagnostic::simple(format!(
                            "unknown class or missing field '{}'",
                            field_name
                        )));
                    }
                } else {
