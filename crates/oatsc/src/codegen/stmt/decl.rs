...
impl<'a> crate::codegen::CodeGen<'a> {
    ...
    fn lower_array_destructuring(
        ...
        let rest_pos = arr_pat
            .elems
            .iter()
            .position(|opt| matches!(opt, Some(Pat::Rest(_))));
        ...
        for prop in &obj_pat.props {
            match prop {
                ...
                ObjectPatProp::Rest { arg, .. } => {
                    ...
                    let meta_ptr = self
                        .safe_gep(self.i64_t, rest_obj_ptr, &[meta_offset], "rest_meta_ptr")
                        .map_err(|_| Diagnostic::simple_boxed(Severity::Error, "gep failed"))?;
