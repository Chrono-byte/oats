...
fn numeric_array_loop_sum_uses_array_get_f64() -> Result<()> {
    ...
    let src = "export function main(): number { let a: number[] = [1,2,3,4]; let x: number = a[2]; println(x); return 0; }";
    let tmp_path =
...
fn pointer_array_index_and_set_uses_ptr_helpers() -> Result<()> {
    ...
    let src = "export function main(): number { let a: string[] = [\"x\", \"y\"]; let p: string = a[1]; return 0; }";
