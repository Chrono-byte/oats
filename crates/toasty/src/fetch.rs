...
fn get_latest_compiler_tag() -> Result<String> {
    ...
    let response = ureq::get(&url)
        ...
        .call()
        .map_err(|e| Diagnostic::new(format!("Failed to fetch releases: {}", e)))?;

    let releases: serde_json::Value = serde_json::from_reader(response.into_reader())
        .map_err(|e| Diagnostic::new(format!("Failed to parse releases JSON: {}", e)))?;
    ...
    Err(Diagnostic::new("No compiler releases found"))
}
...
fn get_latest_runtime_tag() -> Result<String> {
    ...
    Err(Diagnostic::new("No runtime releases found"))
}
...
fn download_compiler(tag: &str, artifact_name: &str, dest_path: &Path) -> Result<()> {
    ...
    let response = ureq::get(&url)
        .set("User-Agent", "oats-compiler")
        .call()
        .map_err(|e| Diagnostic::new(format!("Failed to download compiler: {}", e)))?;
...
fn download_runtime(tag: &str, artifact_name: &str, dest_path: &Path) -> Result<()> {
    ...
    let response = ureq::get(&url)
        .set("User-Agent", "oats-compiler")
        .call()
        .map_err(|e| Diagnostic::new(format!("Failed to download runtime: {}", e)))?;
